...
AFRAME.registerComponent('markerhandler', {
    init: async function(){
        this.el.addEventListener('markerFound', () => {
            var modelName = this.el.gerAttribute('model_name')
            var barcodeValue = this.el.gerAttribute('value')
            modelList.push({
        ...
        this.el.addEventListener('markerLost', () => {
            var modelName = this.el.getAttibute('model_name')
    ...
    placeTheModel: function(modelName, models){
        ...
        if(isListContainModel){
            ...
            if(distance < 1.25){
                var modelEL = document.setAttribute(`#${modelName}`)
